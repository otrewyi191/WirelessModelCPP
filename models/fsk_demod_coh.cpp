...
FskCoherentDemod::FskCoherentDemod( char* instance_name,
...
//==============================================
FskCoherentDemod::~FskCoherentDemod( void ){ };
//==============================================

void FskCoherentDemod::Initialize(void)
{
...
int FskCoherentDemod::Execute()
{
  ...
  float lo_integ_val, hi_integ_val;
  double max_val=0.0;
  int is;
  #ifdef _DEBUG
    *DebugFile << "In FskCoherentDemod::Execute\0" << endl;
  #endif
  ...
  for (is=0; is<Block_Size; is++)
    ...
    lo_integ_val += std::real(in_val * std::conj(*lo_ref_sig_ptr++));
    hi_integ_val += std::real(in_val * std::conj(*hi_ref_sig_ptr++));
    *lo_integ_sig_ptr++ = lo_integ_val/float(Samps_Per_Symb);
    *hi_integ_sig_ptr++ = hi_integ_val/float(Samps_Per_Symb);
