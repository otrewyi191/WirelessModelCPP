...
FineDelayEstimator::FineDelayEstimator( char* instance_name,
                                PracSimModel* outer_model,
                                Signal<float>* in_sig,
                                Signal<float>* ref_sig,
                                //Signal<float>* out_sig,
                                Control<bool>* estim_enabled_cntl, //input
                                Control<bool>* dly_est_is_valid_cntl, //output
                                Control<float>* estimated_delay_cntl)//output
...
//   Out_Sig = out_sig;
   ...
   Estim_Enabled_Cntl = estim_enabled_cntl;
   Estimated_Delay_Cntl = estimated_delay_cntl;
...
FineDelayEstimator::~FineDelayEstimator( void ){ };

void FineDelayEstimator::Initialize(void)
{
   int dummy_size, i;
   double tmp_nsexp;
...
int FineDelayEstimator::Execute()
{
   using std::complex;
...
//   float max_corr_time;
   ...
   if(!(Estim_Enabled_Cntl->GetValue())) return(_MES_AOK);
...
//   }
      ...
      && (Corr_Pass_Count <= Num_Corr_Passes) )
   {
     ...
     tau_avg = 0.0;
      ...
      if(Corr_Pass_Count == Num_Corr_Passes)
      {
         ...
         est_delay = -phase_to_time * numer / denom;
         Spectrum_File->close();
         Estimated_Delay_Cntl->SetValue(est_delay);
         Dly_Est_Is_Valid_Cntl->SetValue(true);
         #ifdef _DEBUG
            (*DebugFile) << "phase slope found delay as " << est_delay << endl;
         #endif
   ...
   //dft( x, x_of_f, full_corr_size);
//   IfftDitSino<float>(x, full_corr_size);
